...
const precomputed = CASES.map((testCase) => {
  ...
  return { ...testCase, jd, ahargana: jd - KALI_YUGA_EPOCH_JD };
});
...
for (let beeja = -0.65; beeja <= -0.40; beeja += 0.02) {
  ...
  for (let offset = 128; offset <= 138; offset += 0.5) {
    for (let evenCirc = 32.0; evenCirc <= 33.2; evenCirc += 0.1) {
      for (let oddCirc = 31.4; oddCirc <= 32.2; oddCirc += 0.1) {
        const deltas = [];
        for (const testCase of precomputed) {
          ...
          const moonTrue = applyMoonManda(meanMoon, apogee, evenCirc, oddCirc);
        ...
        if (!best || rmse < best.rmse || (Math.abs(rmse - best.rmse) < 1e-9 && maxAbs < best.maxAbs)) {
          best = {
            beeja: Number(beeja.toFixed(2)),
            offset: Number(offset.toFixed(2)),
            evenCirc: Number(evenCirc.toFixed(2)),
            oddCirc: Number(oddCirc.toFixed(2)),
            rmse,
            maxAbs,
            deltas
          };
...
console.log(JSON.stringify({
  beeja: best.beeja,
  offset: best.offset,
  evenCirc: best.evenCirc,
  oddCirc: best.oddCirc,
  rmse: best.rmse,
  maxAbs: best.maxAbs
}, null, 2));
