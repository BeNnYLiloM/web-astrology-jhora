...
function applyMoonManda(meanMoon, moonApogee) {
  const anomaly = normalize360(meanMoon - moonApogee);
  const circumference = 32 - (32 - 31.67) * Math.abs(sinD(anomaly));
  const correction = asinD((circumference * sinD(anomaly)) / 360.0);
  return anomaly < 180 ? normalize360(meanMoon - correction) : normalize360(meanMoon + Math.abs(correction));
}
...
function evaluate(apogeeBeejaArcMinPerYear, apogeeOffset) {
  const apogeeBeeja = (apogeeBeejaArcMinPerYear / 60) * CIVIL_DAYS_MAHAYUGA / (360 * 365.258756);
  const deltas = [];
  for (const testCase of CASES) {
    ...
    const ahargana = jd - KALI_YUGA_EPOCH_JD;
    const meanMoon = meanLongitude(ahargana, REV_MOON, MOON_BEEJA, MOON_OFFSET);
    const apogee = meanLongitude(ahargana, REV_MOON_APOGEE, apogeeBeeja, apogeeOffset);
    const moonTrue = applyMoonManda(meanMoon, apogee);
...
let best = null;
for (let beeja = -0.70; beeja <= -0.20; beeja += 0.01) {
  for (let offset = 124; offset <= 138; offset += 0.25) {
    const result = evaluate(beeja, offset);
    if (!best || result.rmse < best.rmse || (Math.abs(result.rmse - best.rmse) < 1e-9 && result.maxAbs < best.maxAbs)) {
      best = { beeja: Number(beeja.toFixed(2)), offset: Number(offset.toFixed(2)), ...result };
    }
  }
}

console.log(JSON.stringify({ beeja: best.beeja, offset: best.offset, rmse: best.rmse, maxAbs: best.maxAbs }, null, 2));
