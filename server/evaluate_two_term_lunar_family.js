...
const data = CASES.map((chartCase) => {
  ...
  return {
    ...
    sinMPlus2D: sinD(M + 2 * D),
    sinMPlusMs: sinD(M + Ms)
  };
});

const rows = data.map((row) => [1, row.sinMPlus2D, row.sinMPlusMs]);
...
console.log('independent two-term fit');
console.log('weights=' + weights.map((value) => value.toFixed(6)).join(', '));
console.log('rmse=' + rmse.toFixed(2) + "' maxAbs=" + maxAbs.toFixed(2) + "' within5=" + within5 + '/' + residuals.length);
for (let index = 0; index < data.length; index += 1) {
  console.log(data[index].id + ': ' + residuals[index].toFixed(2) + "'");
}

let best = null;
for (let intercept = -1.0; intercept <= 1.0; intercept += 0.1) {
  for (let scale = -2.0; scale <= 2.0; scale += 0.05) {
    const deltas = data.map((row) => {
      const added = intercept + scale * (192 / 60) * row.sinMPlus2D + scale * (-110 / 60) * row.sinMPlusMs;
      return added - row.residualTargetMinutes;
    });
    const rmse2 = Math.sqrt(deltas.reduce((sum, value) => sum + value * value, 0) / deltas.length);
    const maxAbs2 = Math.max(...deltas.map((value) => Math.abs(value)));
    const within52 = deltas.filter((value) => Math.abs(value) <= 5).length;
    if (!best || within52 > best.within5 || (within52 === best.within5 && (maxAbs2 < best.maxAbs || (Math.abs(maxAbs2 - best.maxAbs) < 1e-9 && rmse2 < best.rmse)))) {
      best = { intercept, scale, rmse: rmse2, maxAbs: maxAbs2, within5: within52, deltas };
    }
  }
}

console.log('\nsingle-scale modern-ratio fit');
console.log(JSON.stringify({ intercept: best.intercept, scale: best.scale, rmse: best.rmse, maxAbs: best.maxAbs, within5: best.within5 }, null, 2));
for (let index = 0; index < data.length; index += 1) {
  console.log(data[index].id + ': ' + best.deltas[index].toFixed(2) + "'");
}
