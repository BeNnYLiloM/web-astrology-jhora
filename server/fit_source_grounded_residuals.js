...
const data = CASES.map((testCase) => {
  ...
  const d = calculateDiagnostics(jd, testCase.latitude, testCase.longitude);
  const jhora = (testCase.sign - 1) * 30 + testCase.jhora;
  const residual = normalize180(jhora - d.moon.trueLongitude) * 60;
  const D = normalize180(d.moon.trueLongitude - d.sun.trueLongitude);
  const M = normalize180(d.moon.anomaly);
  const manjulaTerm = cosD(d.sun.trueLongitude - d.moon.apogeeLongitude) * sinD(D);
  const bhaskaraTerm = sinD(2 * D);
  const solarAnom = sinD(d.sun.anomaly);
  const lunarAnom = sinD(M);
  return { id: testCase.id, residual, manjulaTerm, bhaskaraTerm, solarAnom, lunarAnom };
});

const models = {
  manjula_only: (item) => [1, item.manjulaTerm],
  manjula_bhaskara: (item) => [1, item.manjulaTerm, item.bhaskaraTerm],
  manjula_bhaskara_solar: (item) => [1, item.manjulaTerm, item.bhaskaraTerm, item.solarAnom],
  all_four: (item) => [1, item.manjulaTerm, item.bhaskaraTerm, item.solarAnom, item.lunarAnom]
};

for (const [name, basisFactory] of Object.entries(models)) {
  const rows = data.map(basisFactory);
  const targets = data.map((item) => item.residual);
  const weights = solveLeastSquares(rows, targets);
  const preds = data.map((item, i) => rows[i].reduce((sum, value, j) => sum + value * weights[j], 0));
  const residuals = preds.map((value, i) => value - targets[i]);
  ...
  for (let i = 0; i < data.length; i++) {
    console.log(data[i].id.padEnd(12) + ' pred=' + preds[i].toFixed(2).padStart(8) + "' actual=" + data[i].residual.toFixed(2).padStart(8) + "'");
  }
}
