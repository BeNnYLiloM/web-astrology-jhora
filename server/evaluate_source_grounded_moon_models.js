...
function summarize(name, deltas) {
  ...
  for (let i = 0; i < CASES.length; i++) {
    console.log(CASES[i].id.padEnd(12) + deltas[i].toFixed(2).padStart(10) + "'");
  }
}
...
const prepared = CASES.map((testCase) => {
  ...
  const sunMinusApogee = normalize180(d.sun.trueLongitude - d.moon.apogeeLongitude);
  const M = normalize180(d.moon.anomaly);
  return { testCase, d, D, sunMinusApogee, M };
});

const base = prepared.map((item) => signedMinutes(item.d.moon.trueLongitude, item.testCase.sign, item.testCase.jhora));
summarize('current_base', base);

const manjula = prepared.map((item) => {
  const corrMin = -144 * cosD(item.sunMinusApogee) * sinD(item.D);
  const corrected = normalize360(item.d.moon.trueLongitude + corrMin / 60);
  return signedMinutes(corrected, item.testCase.sign, item.testCase.jhora);
});
summarize('manjula_second_eq', manjula);

const bhaskaraVariation = prepared.map((item) => {
  const corrMin = 34 * sinD(2 * item.D);
  const corrected = normalize360(item.d.moon.trueLongitude + corrMin / 60);
  return signedMinutes(corrected, item.testCase.sign, item.testCase.jhora);
});
summarize('bhaskara_variation', bhaskaraVariation);

const combined = prepared.map((item) => {
  const corrMin = -144 * cosD(item.sunMinusApogee) * sinD(item.D) + 34 * sinD(2 * item.D);
  const corrected = normalize360(item.d.moon.trueLongitude + corrMin / 60);
  return signedMinutes(corrected, item.testCase.sign, item.testCase.jhora);
});
summarize('manjula_plus_bhaskara', combined);
