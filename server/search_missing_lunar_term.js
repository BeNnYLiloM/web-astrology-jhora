...
const data = CASES.map((chartCase) => {
  ...
  return {
    ...
    candidateTerms: {
      sin2DMinusM: sinD(2 * D - M),
      sin2DPlusM: sinD(2 * D + M),
      sinD: sinD(D),
      sin3DMinusM: sinD(3 * D - M),
      sinDPlusM: sinD(D + M),
      sin2DMinusMs: sinD(2 * D - Ms),
      sin2DPlusMs: sinD(2 * D + Ms),
      sin3D: sinD(3 * D),
      sin3M: sinD(3 * M),
      cos2D: cosD(2 * D),
      cosM: cosD(M)
...
const results = [];
for (const termName of Object.keys(data[0].candidateTerms)) {
  const rows = data.map((row) => [1, row.candidateTerms[termName]]);
  const targets = data.map((row) => row.residualTargetMinutes);
  const weights = solveLeastSquares(rows, targets);
  const deltas = data.map((row, i) => {
    const added = rows[i][0] * weights[0] + rows[i][1] * weights[1];
    return row.residualTargetMinutes - added;
  });
  ...
  results.push({ termName, weights, rmse, maxAbs, roman: deltas.find((_, i) => data[i].id === 'Roman') });
}
results.sort((a, b) => a.maxAbs - b.maxAbs || a.rmse - b.rmse);
for (const item of results) {
  console.log(JSON.stringify({ term: item.termName, rmse: item.rmse, maxAbs: item.maxAbs, weights: item.weights, romanResidual: item.roman }));
}
