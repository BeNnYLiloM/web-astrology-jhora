...
function applyPrecise(meanLongitude, apogeeLongitude, pe, po, mode) {
  const anomaly = mode === 'mean_minus_apogee'
    ? normalize360(meanLongitude - apogeeLongitude)
    : normalize360(apogeeLongitude - meanLongitude);
  const sinAbs = Math.abs(sinD(anomaly));
  const circ = pe - (pe - po) * sinAbs;
  const P = sinD(anomaly) * (circ / 360.0);
  const B = cosD(anomaly) * (circ / 360.0);
  const H = Math.sqrt((1 + B) * (1 + B) + P * P);
  const corr = asinD(P / H);
  if (mode === 'mean_minus_apogee') {
    return anomaly < 180 ? normalize360(meanLongitude - corr) : normalize360(meanLongitude + Math.abs(corr));
  }
  return normalize360(meanLongitude + corr);
}

for (const mode of ['mean_minus_apogee', 'apogee_minus_mean']) {
  let rms = 0;
  let maxAbs = 0;
  console.log('\\nMode: ' + mode);
  for (const testCase of CASES) {
    ...
    const value = applyPrecise(diagnostic.moon.meanLongitude, diagnostic.moon.apogeeLongitude, 32, 31.67, mode);
    const delta = signedMinutes(value, testCase.sign, testCase.jhora);
    rms += delta * delta;
