...
function fixedMaxMoon(meanLongitudeValue, apogeeLongitude) {
  const anomaly = normalize360(apogeeLongitude - meanLongitudeValue);
  const corr = asinD(sinD(anomaly) * sinD(MAX_EQ));
  return normalize360(meanLongitudeValue + corr);
}
const pre = CASES.map(c=>{const jd=jdFrom(c.date,c.time,c.timezone); return {...c, ahargana: jd-KALI_YUGA_EPOCH_JD};});
let best = null;
for (let beeja = -0.70; beeja <= -0.20; beeja += 0.01) {
  ...
  for (let offset = 124; offset <= 138; offset += 0.25) {
    const deltas = [];
    for (const c of pre) {
      const meanMoon = meanLongitude(c.ahargana, REV_MOON, MOON_BEEJA, MOON_OFFSET);
      const apogee = meanLongitude(c.ahargana, REV_MOON_APOGEE, apogeeBeeja, offset);
      const moon = fixedMaxMoon(meanMoon, apogee);
      deltas.push(signedMinutes(moon, c.sign, c.jhora));
    }
    ...
    if (!best || rmse < best.rmse || (Math.abs(rmse - best.rmse) < 1e-9 && maxAbs < best.maxAbs)) {
      best = { beeja: Number(beeja.toFixed(2)), offset: Number(offset.toFixed(2)), rmse, maxAbs, deltas };
    }
  }
}
