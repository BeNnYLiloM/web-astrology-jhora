...
const data = CASES.map((chartCase) => {
  ...
  const meanMoon = normalize360(diagnostics.moon.meanLongitude + offsetArcMin / 60);
...
const models = {
  generalized_munjala: (item) => [1, item.munjala, item.elong2, item.solar, item.lunar],
  generalized_munjala_plus_evection: (item) => [1, item.munjala, item.elong2, item.solar, item.lunar, item.evection]
};
...
for (const [name, basisFactory] of Object.entries(models)) {
  ...
  console.log('weights=' + weights.map((value) => value.toFixed(4)).join(', '));
  for (let index = 0; index < data.length; index += 1) {
    console.log(data[index].id.padEnd(12) + ' ' + residuals[index].toFixed(2) + "'");
  }
}
