...
let best = null;
for (let offsetArcMin = 0; offsetArcMin <= 3.0; offsetArcMin += 0.25) {
  const rows = [];
  const targets = [];
  const base = [];
  ...
  for (const chartCase of CASES) {
    ...
    const shiftedMoon = normalize360(diagnostics.moon.trueLongitude + offsetArcMin / 60);
    const targetLongitude = (chartCase.sign - 1) * 30 + chartCase.jhora;
    const residual = normalize180(targetLongitude - shiftedMoon) * 60;
    const D = normalize180(shiftedMoon - diagnostics.sun.trueLongitude);
    const M = normalize180(diagnostics.moon.anomaly + offsetArcMin / 60);
    ...
    base.push({ chartCase, shiftedMoon });
  }

  const weights = solveLeastSquares(rows, targets);
  const deltas = rows.map((row, index) => {
    const correctionMinutes = row.reduce((sum, value, termIndex) => sum + value * weights[termIndex], 0);
    const correctedLongitude = normalize360(base[index].shiftedMoon + correctionMinutes / 60);
    return signedMinutes(correctedLongitude, base[index].chartCase.sign, base[index].chartCase.jhora);
  });
  ...
  if (!best || maxAbs < best.maxAbs || (Math.abs(maxAbs - best.maxAbs) < 1e-9 && rmse < best.rmse)) {
    best = { offsetArcMin, weights, rmse, maxAbs, deltas };
  }
}
