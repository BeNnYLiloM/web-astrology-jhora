...
function applyMoonManda(meanMoon, moonApogee, evenCirc, oddCirc) {
  const anomaly = normalize360(meanMoon - moonApogee);
  const circumference = evenCirc - (evenCirc - oddCirc) * Math.abs(sinD(anomaly));
...
let best = null;
for (let evenCirc = 31.0; evenCirc <= 33.5; evenCirc += 0.05) {
  for (let oddCirc = 30.5; oddCirc <= 33.0; oddCirc += 0.05) {
    const deltas = [];
    for (const testCase of CASES) {
      ...
      const moonTrue = applyMoonManda(diagnostic.moon.meanLongitude, diagnostic.moon.apogeeLongitude, evenCirc, oddCirc);
    ...
    if (!best || rmse < best.rmse || (Math.abs(rmse - best.rmse) < 1e-9 && maxAbs < best.maxAbs)) {
      best = { evenCirc: Number(evenCirc.toFixed(2)), oddCirc: Number(oddCirc.toFixed(2)), rmse, maxAbs, deltas };
    }
  }
}

console.log(JSON.stringify({ evenCirc: best.evenCirc, oddCirc: best.oddCirc, rmse: best.rmse, maxAbs: best.maxAbs }, null, 2));
